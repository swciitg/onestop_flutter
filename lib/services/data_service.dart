...
class DataService {
  static Future<Map<String, dynamic>?> getLastUpdated() async {
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.lastUpdated);
    if (cachedData == null) {
      return null;
    }
  ...
  static Future<List<TravelTiming>> getBusTiming() async {
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.busTimings);
    Map<String, dynamic> jsonData;
    if (cachedData == null) {
      jsonData = await TravelRepository().getBusTiming();
      await LocalStorage.instance.storeListRecord([jsonData], DatabaseRecords.busTimings);
    } else {
    ...
    for (int i = 0; i < busTimings[0].weekdays.toCampus.length; i++) {
      busTimings[0].weekdays.toCampus[i] = busTimings[0].weekdays.toCampus[i].toLocal();
    }
    for (int i = 0; i < busTimings[0].weekdays.fromCampus.length; i++) {
      busTimings[0].weekdays.fromCampus[i] = busTimings[0].weekdays.fromCampus[i].toLocal();
    }
    for (int i = 0; i < busTimings[0].weekend.toCampus.length; i++) {
      busTimings[0].weekend.toCampus[i] = busTimings[0].weekend.toCampus[i].toLocal();
    }
    for (int i = 0; i < busTimings[0].weekend.fromCampus.length; i++) {
      busTimings[0].weekend.fromCampus[i] = busTimings[0].weekend.fromCampus[i].toLocal();
    }
    return busTimings;
  }

  static Future<List<RestaurantModel>> getRestaurants() async {
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.restaurant);

    if (cachedData == null) {
      List<Map<String, dynamic>> restaurantData = await FoodRepository().getRestaurantData();
      List<RestaurantModel> restaurants =
          restaurantData.map((e) => RestaurantModel.fromJson(e)).toList();

      await LocalStorage.instance.storeListRecord(restaurantData, DatabaseRecords.restaurant);
  ...
  static Future<List<HomeImageModel>> getHomeImageLinks() async {
    ...
    if (cachedData == null) {
      ...
      imageLinks = (homePageUrls['cardsDataList'] as List).cast<Map<String, dynamic>>();
      await LocalStorage.instance.storeJsonRecord(homePageUrls, DatabaseRecords.homePage);
    } else {
      imageLinks = (cachedData['cardsDataList'] as List).cast<Map<String, dynamic>>();
    }
  ...
  static Future<MealType> getMealData({
    ...
    var cachedData = (await LocalStorage.instance.getListRecord(DatabaseRecords.messMenu))?[0];
    Map<String, dynamic>? jsonData;
  ...
  static Future<SplayTreeMap<String, ContactModel>> getContacts() async {
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.contacts);
    SplayTreeMap<String, ContactModel> people = SplayTreeMap();
    if (cachedData == null) {
      List<Map<String, dynamic>> contactData = await APIRepository().getContactData();
      for (var element in contactData) {
        people[element['sectionName']] = ContactModel.fromJson(element);
      }
      await LocalStorage.instance.storeListRecord(contactData, DatabaseRecords.contacts);
      return people;
    } else {
  ...
  static Future<List<DropdownContactModel>> getDropDownContacts() async {
    ...
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.dropdowncontacts);
    try {
      List<Map<String, dynamic>> dropDownData = await APIRepository().getDropDownContacts();
      await LocalStorage.instance.storeListRecord(dropDownData, DatabaseRecords.medicalcontacts);
  ...
  static Future<Map<String, List<NotifsModel>>> getNotifications() async {
    ...
    Map<String, List<NotifsModel>> output = {"userPersonalNotifs": [], "allTopicNotifs": []};
  ...
  static Future<List<TravelTiming>> getFerryTiming() async {
    var cachedData = await LocalStorage.instance.getListRecord(DatabaseRecords.ferryTimings);
    Map<String, dynamic> jsonData;
    if (cachedData == null) {
      jsonData = await TravelRepository().getFerryTiming();
      await LocalStorage.instance.storeListRecord([jsonData], DatabaseRecords.ferryTimings);
    } else {
      jsonData = cachedData[0] as Map<String, dynamic>;
    }
    ...
    for (var j = 0; j < ferryTimings.length; j++) {
      for (int i = 0; i < ferryTimings[j].weekdays.toCampus.length; i++) {
        ferryTimings[j].weekdays.toCampus[i] = ferryTimings[j].weekdays.toCampus[i].toLocal();
      }
      for (int i = 0; i < ferryTimings[j].weekdays.fromCampus.length; i++) {
        ferryTimings[j].weekdays.fromCampus[i] = ferryTimings[j].weekdays.fromCampus[i].toLocal();
      }
      for (int i = 0; i < ferryTimings[j].weekend.toCampus.length; i++) {
        ferryTimings[j].weekend.toCampus[i] = ferryTimings[j].weekend.toCampus[i].toLocal();
      }
      for (int i = 0; i < ferryTimings[j].weekend.fromCampus.length; i++) {
        ferryTimings[j].weekend.fromCampus[i] = ferryTimings[j].weekend.fromCampus[i].toLocal();
      }
    }
    return ferryTimings;
  }
